...
np.random.seed(1)
def idf(f):   #处理所有数据 得到所有候选触发词的idf
    # text = open('./train.txt', 'r', encoding='utf-8').read()
    text = []  # 存train1中所有的句子 用于找所有的触发词
    for i in f:
        i = i.strip().split()
        text.append(i[-1])
    out = open('./tf_idf/rel_word.txt', 'w', encoding='utf-8')
    for line in text:
        words = pseg.cut(line)
        for word, flag in words:
            if str(flag) == 'n' or str(flag) == 'v' or str(flag) == 'vn':
                out.write(str(word) + ' ')
        out.write('\n')
    N = len(text)
    out.close()
    #   求IDF
    f = open('./tf_idf/rel_word.txt', 'r', encoding='utf-8')
    # idf = open('./tf_idf/idf.txt', 'w', encoding='utf-8')
    lines = f.readlines()
    t = []
    for line in lines:
        ...
        for i in line:
            t.append(i)
    t = set(t)  # 去掉重复的触发词 一共有1770个关系词
    print('训练集去重之后一共有多少个触发词', len(t))
    id_freq = {}
    for i in t:
        for line in lines:
            line = line.strip().split()
            for j in line:
                while i == j:
                    id_freq[i] = id_freq.get(i, 0) + 1
                    break
    with open('./tf_idf/idf.txt', 'w', encoding='utf-8') as f_idf:
        for key, value in id_freq.items():
            f_idf.write(key + ' ' + str(math.log(N / value, 2)) + '\n')
    f_idf.close()
    # 得到所有触发词IDF的均值
    f_idf = open('./tf_idf/idf.txt', 'r', encoding='utf-8')
    cut = 0
    idf_freq = {}
    mean_idf = 0.0  # 均值
    for line in f_idf:
    ...
    mean_idf = sum(idf_freq.values()) / cut
    return idf_freq,mean_idf


def tf_idf(sentence,idf_freq,mean_idf):  #处理哈工大之后触发词为BLANK的句子
    # # text = open('./train.txt', 'r', encoding='utf-8').read()
    # text =[]  #存train1中所有的句子 用于找所有的触发词
    # for i in f:
    #     i = i.strip().split()
    ...
    # mean_idf = sum(idf_freq.values()) / cut
    #  处理要处理的句子
    print('哈工大之后触发词为BLANK的句子:',sentence)  #注入的骨水泥稳定了椎体，从而减少了神经根受压
    # f_test = open('./test.txt', 'r', encoding='utf-8')
    # lines = f_test.read()
    # lines = lines.strip().split()
    t1 = []  #保存句子中的候选触发词
    freq = {}  #保存句子中候选触发词以及对应的sorce
    # for line in lines:
    ...
    for k in freq:  # 计算TF-IDF
        freq[k] *= idf_freq.get(k, mean_idf) / total
    tags = sorted(freq, key=freq.__getitem__, reverse=True)  # 降序排列所有触发词的sorce
    print('降序排列之后的触发词：',tags)
    return tags[0]  #取第一个
...
def get_sym():
    ...
    lines = f3.readlines()
    idf_freq,mean_idf = idf(lines)   #  idf_freq 数据中所有的触发词的idf 。 mean_idf 所有idf的均值
    for line in lines:
        line =line.strip('\n')
        str = line.split()
        # print(str[-1])
        ...
        if len(rel_word)==0:
            # s ='BLANK'
            s = tf_idf(str[-1],idf_freq,mean_idf)
        if len(rel_word)==1:
            s=rel_word[0]
        if len(rel_word)>1:
            s = rel_word[0]
